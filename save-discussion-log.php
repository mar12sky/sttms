...
if (!empty($_POST)) {
    $stmt = pdo_connect_mysql()->prepare('INSERT INTO agenda_meta (agenda_meta_id, agenda_id, del_id, party_time_allotted, party_time_taken, time_allotted, time_taken, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
    $stmt->execute([NULL, $agenda_id, $del_id, $party_time_allotted, $party_time_taken, $time_allotted, $time_taken, $created]);
    $data = [
        "status" => "success",
